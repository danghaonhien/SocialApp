...
router.get("/me", auth, async (req, res) => {
  try {
    ...
    if (!profile) {
      return res.status(420).json({ msg: "There is no profile for this user" });
    }
    res.json(profile);
  } catch (err) {
...
//   const profileExp = {};
//   profileExp.user = req.user.id;
//   profileExp.experience = {};

//   try {
//     let profile = await Profile.findOne({ user: req.user.id });
//     // const updateExp = profile.experience.map((exp) => exp.id).indexOf(expId);
//     if (profile) {
//       profile = await Profile.findOneAndUpdate(
//         { expId },
//         {
//           $set: profileExp.experience,
//         },
//         { new: true }
//       );
//     }
...
router.put(
  ...
  [
    ...
    [
      ...
      check("current", "Current of study is required").not().isEmpty(),
    ],
  ],
  async (req, res) => {
    const errors = validationResult(req);
